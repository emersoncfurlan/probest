...
public class main {
	...
	public static void main(String[] args) {
		...
		while (opcao != SAIR_DO_PROGRAMA) {
			...
			case MEDIA_HARMONICA:
				...
				break;
			case PRODUTO:
				JOptionPane.showConfirmDialog(null, produto(amostra)); // pronto
				break;
			case PROBABILIDADE:
				JOptionPane.showConfirmDialog(null, probabilidade(amostra, possibilidade));
				break;
			case FATORIAL:
				String texto="Informe o número para o fatorial:";
				String num=JOptionPane.showInputDialog(null, texto);
				int numx= Integer.parseInt(num);
				JOptionPane.showConfirmDialog(null, fatorial(numx)); // feito
				break;
	...
	private static double probabilidade(int[] amostra, int possibilidade) {
		...
		// possíveis:
		double numTotal = fatorial(amostra.length);
	...
	// Fatorial recursivo
	private static double fatorial(int numx) {
		double fatorial = numx;
		if (numx <= 0) {
			return 0;
		} else {
			do {
